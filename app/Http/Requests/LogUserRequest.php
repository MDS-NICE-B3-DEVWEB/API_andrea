...
class LogUserRequest extends FormRequest
{
    ...
    {
        return [
            'email'=>'required|email|exists:users,email', /* le |email permet de vérfier que c'est un champ/format email et exists:users,email permet de vérifier que l'email est bien dans la table users*/
            'password'=>'required'
        ];
    }
